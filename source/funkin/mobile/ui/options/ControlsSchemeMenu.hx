...
*/
var hitboxShowcases<HitboxShowcase>;
...
Touch X position when touch was just pressed.
...
Touch X distance between dragStartingX and current touch position.
...
Represents the background shader for the menu,

utilizing HSV color adjustments.
*/
...
Sets up every needed camera.
*/
function setupCameras()
{
final mainCamera = new FunkinCamera('SchemeMenuCamera');
...
Sets up the hitbox showcase items.
*/
...
for (i in 0...availableSchemes.length)
{
  final hitboxShowcase:HitboxShowcase = new HitboxShowcase(
    0,
    0,
    i,
    currentIndex,
  ...
  hitboxShowcase.x = Math.floor(
    FlxG.width * -0.16
    + (1500 * (i * FullScreenScaleMode.wideScale.x))
  );
...
itemNavHitbox = new FunkinSprite(
  FlxG.width * 0.295
).makeSolidColor(
...
@param isDemoScreen Returns true if player is currently

in hitbox demo.
*/
function createButton(isDemoScreen)
{
...
{
  currentButton = new SchemeMenuButton(
    FlxG.width * 0.83,
    FlxG.height * 0.03,
    'BACK',
    onHitboxDemoBack
...
Called when the current button is pressed

and player is not in demo right now.
*/
 ...
 *
 * The old code attempted to access:
 *
 * hint.deadZones
 *
 * but FunkinHint no longer contains that field.
 *
 * The hitbox demo works without registering
 * the button as a DeadZone.
 */
...
and player is in demo right now.
*/
...
@param index Used to change currentIndex.
*/
...
{
dragStartingX = TouchUtil.touch.x;
}
...
{
  dragDistance =
    TouchUtil.touch.x - dragStartingX;
}
...
HitboxShowcases X position when the player

just pressed on the screen.
*/
var originX;
